...
export function ServicesFilter({ services, categories }: ServicesFilterProps) {
  ...
  // Estado inicial desde la URL (deep-link desde el buscador del hero).
  const catParam = searchParams.get("cat") ?? "all";
  const validCat =
    catParam === "all" || categories.some((c) => c.id === catParam)
      ? catParam
      : "all";

  const [activeCategory, setActiveCategory] = useState<string>(validCat);
  ...
  return (
    <div className="container mx-auto px-4 py-8 md:py-10">
      ...
      <div className="max-w-xl mx-auto mb-6">
        <div className="flex items-center gap-2 rounded-full bg-white border border-slate-light shadow-sm px-4 py-2 focus-within:border-blue-primary focus-within:ring-2 focus-within:ring-blue-primary/20 transition">
          ...
          {query && (
            <button
              ...
              className="shrink-0 text-slate-muted hover:text-slate-dark transition-colors"
            >
